...
def add_trades(fills: pd.DataFrame) -> pd.DataFrame:
    ...
    # First
    fills.loc[fills['split_fill'] == 'first', 'size'] = np.sign(fills.loc[fills['split_fill'] == 'first', 'size']) * (np.abs(fills.loc[fills['split_fill'] == 'first', 'size']) - np.abs(fills.loc[fills['split_fill'] == 'first', 'delta']))
