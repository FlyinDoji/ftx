...
def get_spot_summary(spot_fills: pd.DataFrame) -> pd.DataFrame:
    ...
    spot_summary = pd.DataFrame(columns=['Volume','TakerVolume','MakerVolume','SellVolume','BuyVolume','Fees',])
    ...
    for market in active_markets:
        ...
        market_summary = pd.Series({
            ...
            "Fees":active_market_df.apply(lambda row: row.fee * row.price 
                                            if row.feeCurrency != fee_currency
                                            else row.fee, axis=1).sum().round(4),
            "FeeCurrency": fee_currency,
            },name=market
        )
       
        spot_summary = spot_summary.append(market_summary)
        active_market_df.apply(lambda row: row.fee * row.price 
                                                   if row.feeCurrency != fee_currency 
                                                   else row.fee, axis=1)
...
def get_futures_trades_by_market(fills_by_market: pd.DataFrame, funding: pd.DataFrame) -> pd.DataFrame:
    ...
    for future, df in fills_by_market.items():
        ...
        trades = pd.concat([raw_pnl,fees,volume, executions,start_time,end_time,duration,], axis=1)
        ...
        trades = trades[['raw_pnl','fee','funding','rpnl','executions','volume','start','end','duration',]]
